-- Consolidated schema for a fresh Supabase project (hyvfwkfrnfzdnisaqfpp).
-- Paste this whole file into the Supabase SQL Editor and click "Run" once.
-- It's the concatenation of supabase/migrations/*.sql in order — kept here
-- only as a convenience for manual bootstrap when the CLI/DB password isn't
-- available. The migrations/ folder remains the source of truth.
...
-- Migration 2: tighten tg_set_updated_at search_path (superseded by re-create above, kept for parity)
revoke execute on function public.has_role(uuid, app_role) from anon;
revoke execute on function public.handle_new_user() from anon, authenticated;
revoke execute on function public.tg_set_updated_at() from anon, authenticated;

-- Migration 3: language support
