...
const VARIETY_NOTE = `Держи максимальное разнообразие: рифмы, брейнрот, мемы, шутки и реакции должны быть КАЖДЫЙ РАЗ новыми и своими. Не повторяй прошлые формулировки и не крути по кругу один и тот же набор фраз. Айсековские термины — только по делу и в меру, не в каждом сообщении. НИКОГДА не используй тире «—»/«–» — запятая или точка вместо него.`;
