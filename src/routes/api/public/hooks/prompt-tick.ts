    ...
    handlers: {
      POST: async () => {
        ...
        const { data: chats } = await supabase
          .from("chats")
          .select("id, telegram_chat_id, bot_settings(prompt_frequency, prompt_hour_utc, prompts_enabled, quiet_start, quiet_end, language)")
          .eq("is_active", true);

        const { data: prompts } = await supabase.from("prompts").select("text, language").eq("is_active", true);
        ...
        for (const chat of chats ?? []) {
          ...
          const lang: Lang = s.language === "ru" ? "ru" : s.language === "en" ? "en" : "en";
